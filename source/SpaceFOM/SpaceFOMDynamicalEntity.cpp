/*!
@file SpaceFOM/SpaceFOMDynamicalEntity.cpp
@ingroup SpaceFOM
...
@trick_link_dependency{SpaceFOMDynamicalEntity.cpp}
...
#include "SpaceFOM/SpaceFOMDynamicalEntity.hh"
 ...
 */
SpaceFOMDynamicalEntity::SpaceFOMDynamicalEntity() // RETURN: -- None.
   : dynamical_data( NULL )
{
}
 ...
 * @job_class{initialization}
 */
void SpaceFOMDynamicalEntity::initialize(
   ...
   DynamicalEntityData * dynamics_data_ptr  )
{
   ...
   if ( dynamics_data_ptr == NULL ) {
      errmsg << "SpaceFOM::SpaceFOMDynamicalEntity::initialize():" << __LINE__
   ...
   SpaceFOMPhysicalEntity::initialize(physical_data_ptr);
...
void SpaceFOMDynamicalEntity::pack()
{

   // Call the SpaceFOMPhysicalEntity pack routine.
   SpaceFOMPhysicalEntity::pack();

   // Print out debug information if desired.
   if ( debug ) {
      cout.precision( 15 );
      cout << "SpaceFOMDynamicalEntity::pack():" << __LINE__ << endl
           << "\mass: " << mass << endl
...
void SpaceFOMDynamicalEntity::unpack()
{

   // Call the SpaceFOMPhysicalEntity pack routine.
   SpaceFOMPhysicalEntity::unpack();
   ...
   if ( entity_attr->is_received() ) {
      ...
      // FIXME:
      // Missing data here and possibly in SpaceFOMPhysicalEntity.
