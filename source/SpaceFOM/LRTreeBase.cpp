...
bool LRTreeBase::build_tree()
{
   ...
   for ( unsigned int iinc = 0 ; iinc < this->nodes.size() ; iinc++ ) {
      ...
      for ( unsigned int jinc = 0 ; jinc <= iinc ; jinc++ ) {
         // Any diaganol elements just have themselves.
         if ( jinc == iinc ){
 ...
 * @job_class{scheduled}
 */
LRTreeNodeVector * LRTreeBase::get_path_to_root( unsigned int node_id )
{
   LRTreeNodeBase   * node_ptr;
 ...
 * @job_class{scheduled}
 */
LRTreeNodeVector * LRTreeBase::find_path( unsigned int local,
                                          unsigned int wrt   )
{
   LRTreeNodeVector *up_path = NULL;
   LRTreeNodeVector *down_path = NULL;
   LRTreeNodeBase   *common_node = NULL;
   ...
   if (    this->has_node( local )
      ...
      if (    up_path != NULL 
         ...
         common_node = find_common_node( up_path, down_path );
         ...
         if ( common_node != NULL ) {
            ...
            // First allocate a retrun path vector.
 ...
 * @job_class{scheduled}
 */
LRTreeNodeBase * LRTreeBase::find_common_node( unsigned int local,
                                               unsigned int wrt   )
{
   LRTreeNodeVector *up_path = NULL;
   LRTreeNodeVector *down_path = NULL;
