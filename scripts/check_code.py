...
def main() :
   ...
   ccpcheck_output_dir              = 'cppcheck'
   cppcheck_build_dir               = ccpcheck_output_dir + '/build'
...
Option -a: Autogenerated error suppression file: ''' + \
ccpcheck_output_dir + '/' + ccpcheck_suppresion_autogen_file  ))
   ...
   if args.clean_gen_files :
      if cleanup_autogen_directory( ccpcheck_output_dir, args.test_only, args.very_verbose ) :
   ...
   if os.path.isdir( ccpcheck_output_dir ) :
      ...
      if args.test_only :
         TrickHLAMessage.status( 'Would create the \'' + trickhla_home + '/'
                                 + ccpcheck_output_dir + '\' directory.' )
      else :
         if args.very_verbose :
            TrickHLAMessage.status( 'Creating \'' + trickhla_home + '/' 
                                    + ccpcheck_output_dir + '\' directory.' )
         # Create the cppcheck output directory.
         os.mkdir( ccpcheck_output_dir )
   ...
   if not args.check_includes :
      # Ignore/suppress the HLA Evolved header file warnings.
   ...
   if args.autogen :
      ...
      # Set the output file.
      if args.generate_xml :
         output_file = ccpcheck_output_dir + '/' + cppcheck_output_xml_file
      else :
         output_file = ccpcheck_output_dir + '/' + cppcheck_output_text_file
   ...
   if args.verbose :
      ...
      TrickHLAMessage.status( 'ccpcheck_output_dir   = ' + ccpcheck_output_dir )
...
def find_trick( trick_path=None, verbose = True ) :
   ...
   try :
      ...
      TrickHLAMessage.error( subprocess.error )
      TrickHLAMessage.failure( '\'trick-version -y\' command failed!: '\
                               + trick_version_cmd )
...
# @param  cppcheck_dir The cppcheck autogen directory to clean up.
# @param  test_only    Flag indicating to only show what would be done.
# @param  verbose      Flag to set if verbose outputs are on.
#
def cleanup_autogen_directory(
   ...
   if verbose :
      TrickHLAMessage.status( 'Cleaning up the cppcheck autogen directory!: ' 
                              + cppcheck_dir)

   # Check for the cppcheck autogen directory.
   if os.path.isdir( cppcheck_dir ) :
      if test_only :
         TrickHLAMessage.status( 'Would remove \'' + cppcheck_dir + '\' directory.' )
      else :
         if verbose :
            TrickHLAMessage.status( 'Removing \'' + cppcheck_dir + '\' directory.' )
         # This is not a test, so remove the directory.
         for root, dirs, files in os.walk( cppcheck_dir ) :
            # Remove all the files in the cppcheck directory.
