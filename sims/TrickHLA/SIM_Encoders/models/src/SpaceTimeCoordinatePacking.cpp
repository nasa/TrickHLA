/*!
@file SpaceTimeCoordinatePacking.cpp
@ingroup SpaceFOM
@brief This class provides an example class for testing the encoding and
decoding of the SpaceFOM SpaceTimeCoordinateState data type.
   ...
   TrickHLA::Object *obj )
{
   ...
   stc_attr = get_attribute_and_validate( "state" );
 ...
 * @job_class{initialization}
 */
void SpaceTimeCoordinatePacking::publish()
{
   if ( this->initialized ) {
      ostringstream errmsg;
      errmsg << "SpaceTimeCoordinatePacking::publish():" << __LINE__
 ...
 * @job_class{initialization}
 */
void SpaceTimeCoordinatePacking::subscribe()
{
   if ( this->initialized ) {
      ostringstream errmsg;
      errmsg << "SpaceTimeCoordinatePacking::subscribe():" << __LINE__
             << " WARNING: Ignoring, already initialized!\n";
      message_publish( MSG_WARNING, errmsg.str().c_str() );
   } else {
 ...
 * @job_class{scheduled}
 */
void SpaceTimeCoordinatePacking::pack()
{
   ...
   if ( debug ) {
      ostringstream msg;
      msg << "SpaceTimeCoordinatePacking::pack():" << __LINE__ << '\n';
      print_data( msg );
...
void SpaceTimeCoordinatePacking::unpack()
{
   ...
   if ( debug ) {
      ostringstream msg;
      msg << "SpaceTimeCoordinatePacking::unpack():" << __LINE__ << '\n';
      print_data( msg );
      message_publish( MSG_NORMAL, msg.str().c_str() );
   }
 ...
 * @job_class{scheduled}
 */
void SpaceTimeCoordinatePacking::pack_from_working_data()
{
   int iinc;
   ...
   for ( iinc = 0; iinc < 3; ++iinc ) {
      packing_data.pos[iinc] = working_data->pos[iinc];
      packing_data.vel[iinc] = working_data->vel[iinc];
   }
   // Attitude quaternion.
   packing_data.att.scalar = working_data->att.scalar;
   for ( iinc = 0; iinc < 3; ++iinc ) {
      packing_data.att.vector[iinc] = working_data->att.vector[iinc];
      packing_data.ang_vel[iinc]    = working_data->ang_vel[iinc];
   }
   // Time tag for this state data.
   packing_data.time = working_data->time = get_scenario_time();
 ...
 * @job_class{scheduled}
 */
void SpaceTimeCoordinatePacking::unpack_into_working_data()
{
   ...
   if ( stc_attr->is_received() ) {

      // Unpack the data.
      // Position and velocity vectors.
      for ( int iinc = 0; iinc < 3; ++iinc ) {
         working_data->pos[iinc] = packing_data.pos[iinc];
         working_data->vel[iinc] = packing_data.vel[iinc];
      }
      // Attitude quaternion.
      working_data->att.scalar = packing_data.att.scalar;
      for ( int iinc = 0; iinc < 3; ++iinc ) {
         working_data->att.vector[iinc] = packing_data.att.vector[iinc];
         working_data->ang_vel[iinc]    = packing_data.ang_vel[iinc];
      }
      // Time tag for this state data.
      working_data->time = packing_data.time;
   }
 ...
 * @job_class{scheduled}
 */
void SpaceTimeCoordinatePacking::print_data( std::ostream &stream ) const
{
