...
#      define TRICKHLA_VALIDATE_FPU_CONTROL_WORD                                                    \
         {                                                                                          \
            ...
            if ( ( _fpu_cw & _FPU_PC_MASK ) != ( __fpu_control & _FPU_PC_MASK ) ) {                 \
               message_publish( MSG_WARNING, "%s:%d WARNING: We have detected the \
current Floating-Point Unit (FPU) Control-Word Precision-Control value (%#x: %s) \
does not match the Precision-Control value at program startup (%#x: %s). The change \
in FPU Control-Word Precision-Control could cause the numerical values in your \
simulation to be slightly different in the 7th or 8th decimal place. Please contact \
the TrickHLA team for support.\n",                                                                  \
                                __FILE__, __LINE__,                                                 \
